        ...
        if (rows[0]) {
          ...
          return {
            ...
            timeout_at: r.timeout_at
              ? toTimestamp(new Date(r.timeout_at))
              : null,
            pair_id: r.pair_id ?? null,
          };
        }
...
async findPartnerFlexibleMany(anchor: TicketDbRow, limit = 20): Promise<TicketDbRow[]> {
  ...
  async cancelWithPartnerRecovery(
    ...
    cancelled: TicketDbRow | null;
    partnerRequeued: TicketDbRow | null;
  }> {
    const pool = await getConnectionPool();
    const client = await pool.connect();
    ...
    try {
      ...
      if (t.status === "MATCHED") {
        ...
        await client.query("commit");
        return {
          cancelled: cancelled ? toTicket(cancelled) : null,
          partnerRequeued,
        };
      }
